...
@implementation UTXMLDictionary

#pragma mark - Public API

+ (NSDictionary *)dictionaryFromXMLString:(NSString *)xmlString {
    UTXMLDictionary *parse = [[UTXMLDictionary alloc] init];
...
- (NSMutableDictionary *)attributeDictWithElement:(TBXMLElement *)element {
    ...
    if (attribute == nil) {
        ...
        //_value_在头文件中定义-> UTXMLDictionaryValue
        if (element->text) {
            NSString *value = StringFromChar(element->text);
            if ([value length] > 0) {
                dict[UTXMLDictionaryValue] = value;
            }
        }
    }
    //遍历所有属性
    ...
    while (element) {
        ...
        if (sub) {
            ...
            if ([sub isKindOfClass:[NSDictionary class]]) {
                ...
                //加后缀,以做区分 ?
                NSString *subKey = [firstChildName stringByAppendingString:UTXMLDictionaryArraySubfix];
        ...
        if (bArray) {//当前节点是数组中一个
            ...
            NSString *elementArrayName = [elementName stringByAppendingString:UTXMLDictionaryArraySubfix];
